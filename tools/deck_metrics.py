...
def _consistency_success(style: str, m: dict, dealt_damage: float,
                         won: bool, turns: int) -> bool:
    ...
    if style == "aggro_swarm":
        return m["creatures_played_total"] >= 3 and (
            (won and turns <= 12) or dealt_damage >= 15
        )
    if style == "ramp_level":
...
def build_deck_metrics_report(
    ...
    lines = [
        ...
        "DECK METRICS REPORT",
        "=" * 96,
        "Métricas por deck: WR, consistência, velocidade, resiliência/interação e eficiência de recursos.",
        "",
    ]
